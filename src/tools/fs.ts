...
export function registerFsTools(server: McpServer) {
  server.tool(
    'fs.listDir',
    'List directory entries (files and folders).',
    { dir: z.string() },
    async ({ dir }) => {
  ...
  server.tool(
    'fs.createFile',
    'Create a text/binary file. Set overwrite=true to replace.',
    { file: z.string(), content: z.string().default(''), overwrite: z.boolean().default(false) },
  ...
  server.tool(
    'fs.readFile',
    'Read a file. Optionally limit bytes.',
    { file: z.string(), maxBytes: z.number().int().positive().optional() },
    async ({ file, maxBytes }) => {
  ...
  server.tool(
    ...
    { file: z.string(), content: z.string(), append: z.boolean().default(false) },
    async ({ file, content, append }) => {
      await fs.mkdir(path.dirname(file), { recursive: true });
  ...
  server.tool(
    'fs.createNotebook',
    'Create a minimal Jupyter .ipynb notebook at path.',
    { file: z.string(), title: z.string().default('Notebook'), code: z.string().default('print("hello from MCP")') },
    async ({ file, title, code }) => {
      const nb = {
        cells: [
